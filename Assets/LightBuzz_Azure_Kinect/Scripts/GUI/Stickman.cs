    ...
    public class Stickman : MonoBehaviour
    {
        ...
        public void Load(Body body)
        {
            for (int i = 0; i < _points.Length; i++)
            {
                ...
                if (joint.TrackingState != TrackingState.None && joint.TrackingState != TrackingState.Count)
                {
            ...
            for (int i = 0; i < _bones.Length; i++)
            {
                ...
                if (joint1.TrackingState != TrackingState.None && joint1.TrackingState != TrackingState.Count &&
                    joint2.TrackingState != TrackingState.None && joint2.TrackingState != TrackingState.Count)
                {
        ...
        public void Load(Body body, UniformImage image)
        {
            for (int i = 0; i < _points.Length; i++)
            {
                ...
                if (joint.TrackingState != TrackingState.None && joint.TrackingState != TrackingState.Count)
                {
                    Vector2 point = _space == CoordinateSpace.Color ? joint.PositionColor : joint.PositionDepth;

                    if (point != Vector2.zero)
                    {
                        _points[i].gameObject.SetActive(true);
                        _points[i].localPosition = image.GetPosition(point);
                    }
                    else
                    {
            ...
            for (int i = 0; i < _lines.Length; i++)
            {
                ...
                Joint joint1 = body.Joints[pair.Item1];
                Joint joint2 = body.Joints[pair.Item2];
